...
class TripPlanner (TRIP_PLANNER):
    ...
    def plan_route(self, src_lat, src_lon, dst_name):
        ...
        let todo = BinHeap(self.num_dests * 2, λ x, y: x[1] < y[1])
    ...
    def find_nearby(self, src_lat, src_lon, dst_name, n):
        ...
        let pq = BinHeap(self.num_dests * 2, λ x, y: x[1] < y[1])
        let dist = 0
...
test 'their tests':
    ...
    result = tp.plan_route(0, 0, 'Old North Church')
    assert Cons.to_vec(result) \
      == [[0, 0], [2, 1], [6, 0]]
    tp = TripPlanner(
      [[0, 0, 1.5, 0],
      ...
      [[1.5, 0, 'bank', 'Union'],
       ...
       [4, 0, 'food', 'Jollibee'],
       [5, 0, 'barber', 'Judy']])
    assert tp.plan_route(0, 0, 'Jollibee') == None
